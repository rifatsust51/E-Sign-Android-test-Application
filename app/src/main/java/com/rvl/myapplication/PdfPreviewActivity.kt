...
class PdfPreviewActivity : AppCompatActivity() {
    ...
    companion object {
        ...
        // ── Certificate config ─────────────────────────────────────────────────
        // Client PFX for mTLS with TSP Gateway only (NOT Keycloak)
        ...
        // ── OkHttp: Keycloak (NO client cert, private CA trust) ────────────────
        /**
         * Used for Keycloak token endpoint ONLY.
         * Trusts system CAs + private CA from server_ca.crt.
         * Does NOT send a client certificate — Keycloak doesn't need mTLS.
         */
        fun buildTrustedOkHttpClient(
            ...
            val x509TrustManager = tmf.trustManagers
                .filterIsInstance<X509TrustManager>()
                .firstOrNull()
                ?: throw IllegalStateException("No X509TrustManager found")

            // NO keyManagers — Keycloak does not need client cert
            val sslContext = SSLContext.getInstance("TLS").apply {
                init(null, arrayOf(x509TrustManager), SecureRandom())
            }

            Log.d(TAG, "Trusted OkHttpClient built (no client cert)")
        ...
        // ── OkHttp: TSP Gateway (WITH client cert + private CA trust) ──────────
        /**
         * Used for TSP Gateway signing endpoint ONLY.
         * Trusts system CAs + private CA from server_ca.crt.
         * Sends client certificate from tspgw_client.pfx for mTLS.
         */
        fun buildMtlsOkHttpClient(
            ...
            // ── 1. Load client certificate (PFX/PKCS12) ───────────────────────
            val pfxResId = context.resources.getIdentifier(
                "tspgw_client", "raw", context.packageName
            )
            require(pfxResId != 0) {
                "res/raw/tspgw_client.pfx not found. Add it to src/main/res/raw/ and rebuild."
            }
            ...
            Log.d(TAG, "PFX loaded. Aliases: ${clientKeyStore.aliases().toList()}")

            val kmf = KeyManagerFactory.getInstance(
                KeyManagerFactory.getDefaultAlgorithm()
            ).apply { init(clientKeyStore, PFX_PASSWORD.toCharArray()) }

            // ── 2. Build combined trust store (system + private CA) ────────────
            val combinedTrustStore = buildCombinedTrustStore(context)
            ...
            // ── 3. Build SSLContext WITH client cert for mTLS ──────────────────
            val sslContext = SSLContext.getInstance("TLS").apply {
                init(kmf.keyManagers, arrayOf(x509TrustManager), SecureRandom())
            }

            Log.d(TAG, "mTLS OkHttpClient built successfully")
        ...
        // ── Combined trust store: system CAs + private CA ──────────────────────
        fun buildCombinedTrustStore(context: Context): KeyStore {

            // ── Step A: Load all system trusted CAs ───────────────────────────
            val systemTmf = TrustManagerFactory.getInstance(
                TrustManagerFactory.getDefaultAlgorithm()
            ).apply { init(null as KeyStore?) }

            val systemX509Tm = systemTmf.trustManagers
                .filterIsInstance<X509TrustManager>()
                .firstOrNull()
                ?: throw IllegalStateException("No system X509TrustManager found")

            // ── Step B: Create new KeyStore seeded with system CAs ────────────
            val combinedStore = KeyStore.getInstance(
                KeyStore.getDefaultType()
            ).apply { load(null, null) }

            systemX509Tm.acceptedIssuers.forEachIndexed { index, cert ->
                combinedStore.setCertificateEntry("system_ca_$index", cert)
            }
            Log.d(TAG, "Loaded ${systemX509Tm.acceptedIssuers.size} system CA(s) into trust store")

            // ── Step C: Add private CA(s) from res/raw/server_ca.crt ──────────
            val serverCaResId = context.resources.getIdentifier(
                "server_ca", "raw", context.packageName
            )

            Log.d(TAG, "server_ca resource ID: $serverCaResId")

            if (serverCaResId != 0) {
                try {
                    context.resources.openRawResource(serverCaResId).use { caStream ->
                        val cf    = CertificateFactory.getInstance("X.509")
                        val certs = cf.generateCertificates(caStream)
                        Log.d(TAG, "Found ${certs.size} certificate(s) in server_ca.crt")
                        certs.forEachIndexed { index, cert ->
                            combinedStore.setCertificateEntry("private_ca_$index", cert)
                            Log.d(
                                TAG,
                                "✅ Added private CA[$index]: " +
                                        (cert as X509Certificate).subjectX500Principal.name
                            )
                        }
                    }
                } catch (e: Exception) {
                    Log.e(TAG, "❌ Failed to load server_ca.crt: ${e.message}")
                    e.printStackTrace()
                }
            } else {
                Log.e(
                    TAG,
                    "❌ server_ca.crt NOT FOUND in res/raw! " +
                            "SSL will fail for private CA servers. " +
                            "Add RVLMgmtCATest.cer + TSP CA to res/raw/server_ca.crt"
                )
            }
        ...
        override fun onReceive(context: Context?, intent: Intent?) {
            ...
            when (intent?.action) {
                ACTION_SHOW_IN_APP_PUSH -> {
                    ...
                    Log.d(TAG, "  is_profile_picker: ${intent.getBooleanExtra("is_profile_picker", false)}")
                    Log.d(TAG, "  is_open_app_picker: ${intent.getBooleanExtra("is_open_app_picker", false)}")
                    Log.d(TAG, "  action_count: ${intent.getIntExtra("action_count", 0)}")
    ...
    private val securePrefs by lazy {
        try {
            ...
            Log.w(TAG, "EncryptedSharedPreferences corrupted, clearing: ${e.message}")
    ...
    override fun onResume() {
        ...
        Log.d(TAG, "PdfPreview receiver registered")
    }
    ...
    private fun checkNotificationListenerPermission() {
        ...
        if (!packages.contains(packageName)) {
            AlertDialog.Builder(this)
                ...
                .setMessage(
                    "To display signature notifications seamlessly inside this app, " +
    ...
    private fun requestServiceRebind() {
        try {
            val componentName =
                ComponentName(this, TicThikNotificationInterceptor::class.java)
     ...
     * Uses buildTrustedOkHttpClient — NO mTLS client cert for Keycloak.
     */
    private fun exchangeCodeForToken(
        ...
        Log.d(TAG, "exchangeCodeForToken: starting token exchange")
        ...
        // ← Use trusted client (no mTLS) for Keycloak token endpoint
        buildTrustedOkHttpClient(this).newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                Log.e(TAG, "Token exchange failure: ${e.message}")
                runOnUiThread {
            ...
            override fun onResponse(call: Call, response: Response) {
                ...
                if (!response.isSuccessful || body == null) {
                    Log.e(TAG, "Token exchange failed: ${response.code} body=$body")
                    runOnUiThread {
                        showError("Token exchange failed: ${response.code}")
                        resetToGuestState()
                    }
                    return
                }
                try {
                    ...
                    val accessToken  = json.optString("access_token").ifBlank { null }
                        ?: run {
                            runOnUiThread { showError("No token"); resetToGuestState() }
                            return
                        }
                    ...
                    val userId       = extractClaim(claimsToken, "sub") ?: run {
                        runOnUiThread { showError("Could not read ID"); resetToGuestState() }
                        return
                    }
                    ...
                    val session = UserSession(
                        userId, fullName, email, accessToken, refreshToken, expiry
                    )
                    saveSessionToPrefs(session)
                    currentSession = session
                    Log.d(TAG, "✅ Token exchange success: userId=$userId name=$fullName")
    ...
    private fun refreshTokenIfNeeded(onResult: (success: Boolean) -> Unit) {
        if (!isTokenExpired()) { onResult(true); return }

        val refreshToken = currentSession?.refreshToken?.ifBlank { null }
        if (refreshToken == null) { onResult(false); return }
        ...
        buildTrustedOkHttpClient(this).newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                Log.e(TAG, "Token refresh failure: ${e.message}")
                onResult(false)
            }
            ...
            override fun onResponse(call: Call, response: Response) {
                ...
                try {
                    ...
                    val newExpiry  = extractExpFromJwt(newAccess)
                        ?: (System.currentTimeMillis() / 1000L + 300L)
    ...
    private fun proceedToSign() {
        val session = currentSession
            ?: run { showError("No active session"); renderGuestUI(); return }
        val pdf = base64PdfDocument
            ?: run { showError("PDF not loaded"); resetToSignState(); return }
        isWaitingForConfirmation.set(true)
        signFileService(pdf, session.userId, dynamicFileName, session.accessToken)
    }

    /**
     * Sign document via TSP Gateway.
     * Uses buildMtlsOkHttpClient — WITH mTLS client cert for TSP Gateway.
     */
    private fun signFileService(
        ...
        val jsonPayload = JSONObject().apply {
            ...
            put("signatureSubType", "PAdES_BASELINE_B")
            put("packaging",        "ENVELOPED")
            put("signAlgorithm",    "SHA256withRSA")
            put("hashAlgorithm",    "SHA256")
            put("userId",           userId)
            put("vSigEnabled",      true)
            put("vSigPage",         1)
            put("vSigXPosition",    170)
            put("vSigYPosition",    430)
        }
        ...
        // ← Use mTLS client for TSP Gateway signing endpoint
        val client = buildMtlsOkHttpClient(
            context            = this,
        ...
        currentSigningCall?.enqueue(object : Callback {
            ...
            override fun onResponse(call: Call, response: Response) {
                if (call.isCanceled()) {
                    Log.d(TAG, "Signing cancelled but response arrived")
                    return
                }
                isWaitingForConfirmation.set(false)
                ...
                try {
                    ...
                    val signedBase64: String? = dataObj?.let { d ->
                        d.optJSONArray("signedData")?.optString(0)?.ifBlank { null }
                            ?: d.optString("signedData").ifBlank { null }
                    } ?: json.optString("signedData").ifBlank { null }

                    if (signedBase64 == null) {
                        runOnUiThread {
                            showError("No signed document in response")
                            resetToSignState()
                        }
                        return
                    }
                    ...
                    runOnUiThread {
                        if (savedFile != null) {
                            ...
                            showError("Signed but failed to save")
                            resetToSignState()
                        }
                    }
                } catch (e: Exception) {
                    Log.e(TAG, "Sign response parse error: ${e.message}")
    ...
    private fun showInAppNotification(intent: Intent) {
        ...
        val isProfilePicker = intent.getBooleanExtra("is_profile_picker", false)
        val isOpenAppPicker = intent.getBooleanExtra("is_open_app_picker", false)
        val actionCount     = intent.getIntExtra("action_count", 0)

        Log.d(TAG,
            "showInAppNotification: title='$title' " +
                    "isProfilePicker=$isProfilePicker " +
                    "isOpenAppPicker=$isOpenAppPicker " +
                    "actionCount=$actionCount"
        )

        // ── Collect action data ────────────────────────────────────────────────
        data class ActionData(val title: String, val pending: PendingIntent?)

        val allActions = mutableListOf<ActionData>()
        for (i in 1..actionCount.coerceAtMost(4)) {
            val actionTitle = intent.getStringExtra("action${i}_title") ?: continue
            val pendingIntent: PendingIntent? =
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU)
                    intent.getParcelableExtra("action${i}_intent", PendingIntent::class.java)
                else
                    @Suppress("DEPRECATION")
                    intent.getParcelableExtra("action${i}_intent") as? PendingIntent
            allActions.add(ActionData(actionTitle, pendingIntent))
            Log.d(TAG, "  action[$i] title='$actionTitle' pending=${pendingIntent != null}")
        }

        // Backward-compat: old-style action1/action2 with no action_count
        if (allActions.isEmpty()) {
            val a1t = intent.getStringExtra("action1_title")
            val a1p: PendingIntent? =
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU)
                    intent.getParcelableExtra("action1_intent", PendingIntent::class.java)
                else @Suppress("DEPRECATION")
                intent.getParcelableExtra("action1_intent") as? PendingIntent
            if (a1t != null) allActions.add(ActionData(a1t, a1p))

            val a2t = intent.getStringExtra("action2_title")
            val a2p: PendingIntent? =
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU)
                    intent.getParcelableExtra("action2_intent", PendingIntent::class.java)
                else @Suppress("DEPRECATION")
                intent.getParcelableExtra("action2_intent") as? PendingIntent
            if (a2t != null) allActions.add(ActionData(a2t, a2p))
        }
        ...
        val contentIntent: PendingIntent? =
            ...
            intent.getParcelableExtra("contentIntent") as? PendingIntent

        val density = resources.displayMetrics.density

        // ── Root overlay (dim background) ──────────────────────────────────────
        ...
        val cardView = LinearLayout(this).apply {
            ...
            layoutParams = FrameLayout.LayoutParams(
                ...
                setMargins(
                    (24 * density).toInt(), (48 * density).toInt(),
                    (24 * density).toInt(), (48 * density).toInt()
                )
            }
        }
        ...
        val iconEmoji   = if (isProfilePicker || isOpenAppPicker) "👤" else "🔐"
        val badgeColor  = if (isProfilePicker || isOpenAppPicker) "#EFF6FF" else "#FFF7ED"
        val badgeBorder = if (isProfilePicker || isOpenAppPicker) "#93C5FD" else "#FDBA74"

        val iconBadge = FrameLayout(this).apply {
            background = android.graphics.drawable.GradientDrawable().apply {
                shape = android.graphics.drawable.GradientDrawable.OVAL
                setColor(Color.parseColor(badgeColor))
                setStroke((2 * density).toInt(), Color.parseColor(badgeBorder))
            }
            val size = (80 * density).toInt()
            layoutParams = LinearLayout.LayoutParams(size, size)
                .apply { bottomMargin = (24 * density).toInt() }
            addView(TextView(this@PdfPreviewActivity).apply {
                text      = iconEmoji
                textSize  = 36f
                gravity   = Gravity.CENTER
                layoutParams = FrameLayout.LayoutParams(
        ...
        fun makeBlueBtn(label: String): TextView = TextView(this).apply {
            text     = label
            textSize = 16f
        ...
        fun makeCancelBtn(): TextView = TextView(this).apply {
            text     = "Cancel"
            textSize = 16f
            ...
            background = android.graphics.drawable.GradientDrawable().apply {
                ...
                cornerRadius = 16f * density
                setStroke((2 * density).toInt(), Color.parseColor("#FCA5A5"))
            }
            isClickable = true
            isFocusable = true
            setPadding(
        ...
        when {
            // ── MODE A: Open-app profile picker ───────────────────────────────
            isOpenAppPicker -> {
                Log.d(TAG, "Rendering OPEN-APP profile picker dialog")
                val openBtn = makeBlueBtn("Select Profile in TicThik")
                openBtn.setOnClickListener {
                    dismissWithAnimation {
                        try {
                            contentIntent?.send()
                            Log.d(TAG, "contentIntent fired")
                        } catch (e: Exception) {
                            Log.e(TAG, "contentIntent send failed: ${e.message}")
                            runOnUiThread {
                                showError("Could not open TicThik. Please open it manually.")
                            }
                        }
                        runOnUiThread {
                            btnLoginAndSign.text      = "Waiting for confirmation..."
                            btnLoginAndSign.isEnabled = false
                            tvProfileStatus.text      = "Select profile in TicThik app"
                ...
                buttonSection.addView(openBtn)
                buttonSection.addView(makeCancelBtn().also { btn ->
                    btn.setOnClickListener {
                        dismissWithAnimation { handleSigningDenied() }
                    }
                })
            }

            // ── MODE B: In-app profile picker ─────────────────────────────────
            isProfilePicker && allActions.isNotEmpty() -> {
                Log.d(TAG, "Rendering IN-APP PROFILE PICKER with ${allActions.size} profiles")
                allActions.forEach { actionData ->
                    val profileBtn = makeBlueBtn(actionData.title)
                    profileBtn.setOnClickListener {
                        Log.d(TAG, "Profile selected: '${actionData.title}'")
                        dismissWithAnimation {
                            try {
                                actionData.pending?.send()
                                Log.d(TAG, "Profile intent sent: '${actionData.title}'")
                            } catch (e: Exception) {
                                Log.e(TAG, "Profile send failed: ${e.message}")
                            }
                            runOnUiThread {
                                btnLoginAndSign.text      = "Waiting for confirmation..."
                                btnLoginAndSign.isEnabled = false
                                tvProfileStatus.text      = "Profile selected, awaiting sign..."
                                tvProfileStatus.setTextColor(
                                    getColor(android.R.color.holo_orange_dark)
                                )
                            }
                    ...
                    buttonSection.addView(profileBtn)
                }
                buttonSection.addView(makeCancelBtn().also { btn ->
                    btn.setOnClickListener {
                        dismissWithAnimation { handleSigningDenied() }
            ...
            // ── MODE C: Confirm / Deny ─────────────────────────────────────────
                ...
                Log.d(TAG, "Rendering CONFIRM/DENY dialog")

                var confirmPending: PendingIntent? = null
                var denyPending:    PendingIntent? = null

                val confirmKeywords = listOf("confirm", "yes", "approve", "allow", "sign")
                val denyKeywords    = listOf("deny", "no", "reject", "cancel")

                allActions.forEach { actionData ->
                    when {
                        confirmKeywords.any {
                            actionData.title.contains(it, ignoreCase = true)
                        } -> if (confirmPending == null) confirmPending = actionData.pending

                        denyKeywords.any {
                            actionData.title.contains(it, ignoreCase = true)
                        } -> if (denyPending == null) denyPending = actionData.pending
                    }
                }

                if (confirmPending == null)
                    confirmPending = contentIntent ?: allActions.getOrNull(0)?.pending
                if (denyPending == null)
                    denyPending = allActions.getOrNull(1)?.pending

                Log.d(TAG,
                    "confirmPending=${confirmPending != null} " +
                            "denyPending=${denyPending != null}"
                )
                ...
                btnConfirm.setOnClickListener {
                    Log.d(TAG, "CONFIRM clicked, confirmPending=${confirmPending != null}")
                    dismissWithAnimation {
                        try {
                            confirmPending?.send()
                        ...
                        runOnUiThread {
                            btnLoginAndSign.text = "Signing in progress..."
                        }
                    }
                }

                btnDeny.setOnClickListener {
                    Log.d(TAG, "DENY clicked, denyPending=${denyPending != null}")
                    dismissWithAnimation {
                        try {
                            denyPending?.send()
    ...
    private fun showLogoutConfirmation() {
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Log out") { _, _ ->
                ...
                clearStoredSession()
                currentSession     = null
                cachedSignedFile   = null
                cachedMediaStoreUri = null
                renderGuestUI()
            }
            .setNegativeButton("Cancel", null)
            .show()
    ...
    private fun showPdfImage(filePath: String) {
        try {
            ...
            ivPdfPreview.setImageBitmap(bmp)
            page.close()
            renderer.close()
            fd.close()
        } catch (e: Exception) {
