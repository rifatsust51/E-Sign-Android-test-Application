...
class TicThikNotificationInterceptor : NotificationListenerService() {
    ...
    override fun onNotificationPosted(sbn: StatusBarNotification) {
        ...
        if (actions != null && actions.isNotEmpty()) {
            ...
            for ((index, action) in actions.withIndex()) {
                Log.d("MFA_DEBUG",
                    "  -> Button $index: '${action.title}' | intent=${action.actionIntent != null}")
            }
        } else {
            Log.d("MFA_DEBUG", "No action buttons found.")
        }
        ...
        val isTargetNotification =
            targetPackageName.contains("ticthik",          ignoreCase = true) ||
                    targetPackageName.contains("reliefvalidation", ignoreCase = true) ||
                    title.contains("TickTheek", ignoreCase = true) ||
                    title.contains("TicThik",   ignoreCase = true) ||
                    text.contains("TicThik",    ignoreCase = true)
        ...
        if (anyVisible) {
            ...
            // Cancel system notification BEFORE broadcast so no heads-up appears
            try {
                cancelNotification(sbn.key)
                Log.d("MFA_DEBUG", "System notification cancelled.")
            } catch (e: Exception) {
                Log.e("MFA_DEBUG", "Failed to cancel: ${e.message}")
            }
            ...
            intent.setPackage(applicationContext.packageName)
            sendBroadcast(intent)
            Log.d("MFA_DEBUG", "In-app broadcast SENT.")

        } else {
            Log.d("MFA_DEBUG", "No activity visible — system notification shown.")
        }
    }
}